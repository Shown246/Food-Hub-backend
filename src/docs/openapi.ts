...
const loginResponse = success(ref("LoginPayload"));
const registerResponse = success(ref("RegisterResponse"));
const verifyEmailResponse = success(ref("VerifyEmailResponse"));
const resendVerificationResponse = success(ref("ResendVerificationResponse"));
const forgotPasswordResponse = success(ref("ForgotPasswordResponse"));
const resetPasswordResponse = success(ref("ResetPasswordResponse"));
...
export const openApiDocument = {
  ...
  paths: {
    ...
    "/auth/register": { post: { operationId: "register", tags: ["Authentication"], summary: "Register a customer or provider", description: "Creates user account and dispatches verification email via Resend. Public registration never accepts ADMIN.", requestBody: body(ref("RegisterRequest"), { fullName: "FoodHub Customer", email: "customer@example.com", phone: "+8801700000000", password: "StrongPass1", role: "CUSTOMER" }), responses: { 201: registerResponse, ...errorResponses([400, 409, 422, 429]) } } },
    "/auth/login": { post: { operationId: "login", tags: ["Authentication"], summary: "Log in", description: "Creates a server session. Credential failures are deliberately generic.", requestBody: body(ref("LoginRequest"), { email: "customer@example.com", password: "StrongPass1" }), responses: { 200: loginResponse, ...errorResponses([400, 401, 403, 422, 429]) } } },
    ...
    "/auth/verify-email": {
      post: { operationId: "verifyEmailPost", tags: ["Authentication"], summary: "Verify email with token", description: "Verifies token and marks user account as verified.", requestBody: body(ref("VerifyEmailRequest")), responses: { 200: verifyEmailResponse, ...errorResponses([400, 403, 404, 422, 429]) } },
      get: { operationId: "verifyEmailGet", tags: ["Authentication"], summary: "Verify email from link", description: "Verifies token from query string.", parameters: [query("token", { type: "string" }, "Verification token"), query("callbackURL", { type: "string" }, "Optional redirect URL")], responses: { 200: verifyEmailResponse, ...errorResponses([400, 403, 404, 422, 429]) } },
    },
    "/auth/resend-verification": { post: { operationId: "resendVerification", tags: ["Authentication"], summary: "Resend verification email", description: "Resends verification email to an unverified user account.", requestBody: body(ref("ResendVerificationRequest")), responses: { 200: resendVerificationResponse, ...errorResponses([400, 422, 429]) } } },
    "/auth/forgot-password": { post: { operationId: "forgotPassword", tags: ["Authentication"], summary: "Request password reset email", description: "Dispatches a password reset link to user's registered email address via Resend if the account exists.", requestBody: body(ref("ForgotPasswordRequest")), responses: { 200: forgotPasswordResponse, ...errorResponses([400, 422, 429]) } } },
    "/auth/reset-password": { post: { operationId: "resetPassword", tags: ["Authentication"], summary: "Reset password with token", description: "Resets account password using a single-use verification token and revokes active sessions.", requestBody: body(ref("ResetPasswordRequest")), responses: { 200: resetPasswordResponse, ...errorResponses([400, 403, 404, 422, 429]) } } },
    "/profile": {
    ...
    "/meals": { get: { operationId: "listMeals", tags: ["Meals"], summary: "List publicly orderable meals", parameters: [query("search", { type: "string", maxLength: 100 }), query("category", { type: "string", maxLength: 120 }, "Category slug"), query("categoryId", { type: "string" }, "Category identifier or slug"), query("categorySlug", { type: "string", maxLength: 120 }), query("dietary", { type: "string", maxLength: 50 }), query("provider", { type: "string" }, "Provider identifier"), query("minPrice", ref("Money")), query("maxPrice", ref("Money")), query("sort", { $ref: "#/components/schemas/MealSort" }), ...paginationParameters], responses: { 200: success(arrayOf(ref("Meal")), "Paginated meal list", true), ...errorResponses([400, 422, 429]) } } },
  ...
  components: {
    ...
    schemas: {
      ...
      RegisterResponse: { type: "object", required: ["user", "emailVerified", "verificationEmailSent", "message"], properties: { user: { type: "object", required: ["id", "fullName", "email", "role", "emailVerified"], properties: { id: { type: "string" }, fullName: { type: "string" }, email: { type: "string", format: "email" }, phone: nullable({ type: "string" }), role: ref("Role"), emailVerified: { type: "boolean" } }, additionalProperties: false }, emailVerified: { type: "boolean" }, verificationEmailSent: { type: "boolean" }, message: { type: "string" } }, additionalProperties: false },
      VerifyEmailRequest: { type: "object", required: ["token"], properties: { token: { type: "string" } }, additionalProperties: false },
      VerifyEmailResponse: { type: "object", required: ["verified", "email", "message"], properties: { verified: { type: "boolean" }, email: { type: "string", format: "email" }, message: { type: "string" } }, additionalProperties: false },
      ResendVerificationRequest: { type: "object", required: ["email"], properties: { email: { type: "string", format: "email" } }, additionalProperties: false },
      ResendVerificationResponse: { type: "object", required: ["sent", "message"], properties: { sent: { type: "boolean" }, message: { type: "string" } }, additionalProperties: false },
      ForgotPasswordRequest: { type: "object", required: ["email"], properties: { email: { type: "string", format: "email" } }, additionalProperties: false },
      ForgotPasswordResponse: { type: "object", required: ["sent", "message"], properties: { sent: { type: "boolean" }, message: { type: "string" } }, additionalProperties: false },
      ResetPasswordRequest: { type: "object", required: ["token", "newPassword"], properties: { token: { type: "string" }, newPassword: { type: "string", minLength: 8, maxLength: 128, format: "password" } }, additionalProperties: false },
      ResetPasswordResponse: { type: "object", required: ["reset", "message"], properties: { reset: { type: "boolean" }, message: { type: "string" } }, additionalProperties: false },
      LoginPayload: { type: "object", required: ["user", "providerProfile", "token", "accessToken", "refreshToken"], properties: { user: ref("SafeUser"), providerProfile: nullable(ref("Provider")), token: { type: "string", description: "Session token" }, accessToken: nullable({ type: "string", description: "OAuth access token from the account" }), refreshToken: nullable({ type: "string", description: "OAuth refresh token from the account" }) }, additionalProperties: false, example: { user: { id: "user-id", fullName: "FoodHub Customer", profileImageUrl: null, email: "customer@example.com", phone: "+8801700000000", role: "CUSTOMER", status: "ACTIVE", defaultDeliveryAddress: null, createdAt: "2026-07-19T00:00:00.000Z", updatedAt: "2026-07-19T00:00:00.000Z" }, providerProfile: null, token: "session-token", accessToken: null, refreshToken: null } },
